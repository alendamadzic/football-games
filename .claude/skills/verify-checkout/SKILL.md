...
Single-page Next.js 16 app (bun), at `apps/checkout`. The whole game is client
state under `apps/checkout/src/components/game/`; the only server pieces are
the server actions in `apps/checkout/src/lib/tm/actions.ts` that hit the
Transfermarkt API.
...
```bash
bun install
TM_API_URL=http://localhost:4100 bun run dev --filter checkout   # port 3000
```

`TM_API_URL` (see `apps/checkout/src/lib/tm/client.ts`) lets you point the app
at a local mock so guesses resolve to deterministic appearance counts —
essential for exercising exact score paths (e.g. a player with exactly 180
apps). Serve these routes with any local server:
...
- `/players/{id}/stats` → `{ id, stats: [{ competitionId, competitionName, seasonId, clubId, appearances }] }`
  — `clubId` must match a subject id from `apps/checkout/src/lib/subjects.ts` ("418" = Real Madrid)
